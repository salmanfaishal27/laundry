                             ...
                             echo"<tr>
							  <td>$i</td>
							    <td>$r[nama]</td>
                                    <td>$r[alamat]</td>
									<td>$r[telp]</td>
                                    <td><a class='btn btn-primary' href='?p=olahs&aksi=edit&id=$r[id]'><i class='icon-edit'></i>Edit</a>
...
break;
case "edit":
	$edit = mysql_query("SELECT * FROM supplier WHERE id='$_GET[id]'");
    $r    = mysql_fetch_array($edit);
echo "<form method='post' action='user/prosess.php?act=update' enctype='multipart/form-data'>";
echo "<div class='panel panel-border panel-primary'>
                                    <div class='panel-heading'> 
			...
			<input type='submit' class='btn btn-primary' value='Update'> <a class='btn btn-danger' href='?p=olahs'>Batal</a>
		  </form>
		</div> 
